...
The accepted flow for Regolith is to store configuration scripts and configs inside of the `data` folder. This folder has special support that makes it easy to access during compilation. Read more about the data folder {ref}`here<data-folder>`.
...
You can use the following "runWith" types:
 ...
 - {ref}`exe<executable-filters>`

There is also the {ref}`profile<profile-filters>` filter with slightly different syntax. It lets you nest profiles.
